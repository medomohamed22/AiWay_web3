...
export function requireAdmin(user){if(!user||user.role!=='admin')throw new Error('FORBIDDEN')}
export function cleanText(v,max=500){return String(v??'').trim().slice(0,max)}
export function handleError(error,res,fallback='Server error'){console.error(error);if(error.message==='UNAUTHORIZED')return json(res,401,{error:'Sign in with Pi first'});if(error.message==='INSUFFICIENT_CREDITS')return json(res,402,{error:'Insufficient credits. Buy a Pi package first.'});if(error.message==='FORBIDDEN')return json(res,403,{error:'Admin access required'});return json(res,500,{error:fallback})}
