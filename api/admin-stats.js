import { allowMethods, db, fetchWithTimeout, handleError, json, localize, requestLocale, requireUser, requireAdmin, MARKUP, TOKEN_USD, getPiUsd } from './_lib.js';
...
export default async function handler(req,res){
  ...
  try{
    ...
    const [users,payments,messages,images,conversations,reservations,openRouterInfo]=await Promise.all([
      fetchAll(()=>s.from('users').select('id,pi_uid,username,role,ai_tokens,trial_messages_remaining,has_purchased,last_login_at,created_at,updated_at').eq('role','user').order('created_at',{ascending:false})),
    ...
    for(const im of images){const u=userMap.get(im.user_id);if(u){u.images++;if(new Date(im.created_at)>new Date(u.lastActivity))u.lastActivity=im.created_at}}
    const paidByUser=new Map();for(const p of completed)paidByUser.set(p.user_id,(paidByUser.get(p.user_id)||0)+num(p.usd_amount));
    const usersTable=[...userMap.values()].map(u=>({id:u.id,pi_uid:u.pi_uid,username:u.username,balance:num(u.ai_tokens),purchased:!!u.has_purchased,trialRemaining:num(u.trial_messages_remaining),registeredAt:u.created_at,lastLoginAt:u.last_login_at,lastActivity:u.lastActivity,messages:u.messages,images:u.images,providerCostUsd:u.costUsd,chargedTokens:u.chargedTokens,paidUsd:paidByUser.get(u.id)||0,profitUsd:(paidByUser.get(u.id)||0)-u.costUsd,topModel:[...u.models.entries()].sort((a,b)=>b[1]-a[1])[0]?.[0]||'—'}));
    ...
    const providerCostUsd=assistants.reduce((a,m)=>a+cost(m.token_usage),0);const totalUsd=completed.reduce((a,p)=>a+num(p.usd_amount),0);const totalPi=completed.reduce((a,p)=>a+num(p.amount_pi),0);const issuedPaidTokens=completed.reduce((a,p)=>a+num(p.ai_tokens),0);const remainingUserTokens=users.reduce((a,u)=>a+num(u.ai_tokens),0);
    ...
    return json(res,200,{
      generatedAt:new Date().toISOString(), users:users.length,buyers:users.filter(u=>u.has_purchased).length,purchaseRequests:payments.length,completedPurchases:completed.length,totalPi,totalUsd,currentPiUsd,markup:MARKUP,expectedMarkupPercent:(MARKUP-1)*100,expectedMarginPercent:(1-1/MARKUP)*100,tokenUsd:TOKEN_USD,issuedPaidTokens,soldProviderCapacityUsd:issuedPaidTokens*TOKEN_USD,providerCostUsd,remainingUserTokens,remainingProviderLiabilityUsd:remainingUserTokens*TOKEN_USD,expectedGrossProfitUsd:totalUsd-issuedPaidTokens*TOKEN_USD,realizedGrossProfitUsd:totalUsd-providerCostUsd,realizedGrossProfitPi:totalUsd&&totalPi?(totalUsd-providerCostUsd)*(totalPi/totalUsd):0,
      ...
      messages:{count:assistants.length,costUsd:providerCostUsd,avgCostUsd:assistants.length?providerCostUsd/assistants.length:0,avgChargedTokens:assistants.length?assistants.reduce((a,m)=>a+charged(m.token_usage),0)/assistants.length:0,avgLatencyMs:(()=>{const a=assistants.map(m=>latency(m.token_usage)).filter(Boolean);return a.length?a.reduce((x,y)=>x+y,0)/a.length:0})()},
      ...
      finance:{pendingPayments:pending,failedPayments,topSpenders:usersTable.slice().sort((a,b)=>b.paidUsd-a.paidUsd).slice(0,10),totalBalances:remainingUserTokens},
      ...
      alerts:[
        ...
        ...(remainingUserTokens*TOKEN_USD>(openRouterInfo.credits?.remaining||Infinity)?[{level:'warning',title:'التزام الرصيد أعلى من رصيد المزود',message:'راجع رصيد OpenRouter ورصيد المستخدمين.'}]:[])
      ],
