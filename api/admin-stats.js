...
export default async function handler(req,res){
  ...
  try{
    ...
    const usersTable=[...userMap.values()].map(u=>{const paidUsd=paidByUser.get(u.id)||0;const openRouterReserveUsd=paidUsd/MARKUP;const paidBalanceTokens=u.has_purchased?Math.max(0,num(u.ai_tokens)-TRIAL_TOKENS):0;return {id:u.id,pi_uid:u.pi_uid,username:u.username,balance:num(u.ai_tokens),paidBalanceTokens,purchased:!!u.has_purchased,trialRemaining:num(u.trial_messages_remaining),registeredAt:u.created_at,lastLoginAt:u.last_login_at,lastActivity:u.lastActivity,messages:u.messages,images:u.images,providerCostUsd:u.costUsd,chargedTokens:u.chargedTokens,paidUsd,openRouterReserveUsd,profitUsd:paidUsd-openRouterReserveUsd,topModel:[...u.models.entries()].sort((a,b)=>b[1]-a[1])[0]?.[0]||'—'}});
